...
<div class="container">
    ...
    <div class="row setup-content  {{ $currentStep != 1 ? 'display-none' : ' ' }}" id="step-1">
            ...
            <div class="row setup-content {{ $currentStep != 2 ? 'display-none' : '' }}" id="step-2">
                ...
                echo '
                <div class="  card  shadow  border-bottom mt-1 p-2 mb-5 "><h4 class="m-0  "><h4>'.$value->Question.'</h4>
                <div class="d-flex  flex-row">
                    <div class="form-check form-check-inline ">
                    <input class="form-check-input" type="radio" name="SelectedAnswers['.$key.']"
                    wire:model="SelectedAnswers['.$key.']"  value="'.$this->maybe.'" >
                    <label class="form-check-label" for="SelectedAnswers['.$key.']">ربما</label>
                    </div>

                    <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="SelectedAnswers['.$key.']"
                    wire:model="SelectedAnswers['.$key.']"  value="'.$this->no.'" >
                    <label class="form-check-label" for="SelectedAnswers">لا</label>
                    </div>

                    <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="SelectedAnswers['.$key.']"
                    wire:model="SelectedAnswers['.$key.']"   value="'.$this->yes.'" >
